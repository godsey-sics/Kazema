...
- (UIView *)buildHeaderView{
    ...
    UIFont *headerFont = self.headerTitleFont == nil ? [UIFont systemFontOfSize:15.0] : self.headerTitleFont;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.allowMultipleSelection){
        ...
        if (self.disbleCells)
        {
        [self.indexString enumerateObjectsUsingBlock:^(NSString* obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSAttributedString * data = [self.dataSource czpickerView:self attributedTitleForRow:indexPath.row];
            if ([obj isEqualToString:data.string]) {
                cell.accessoryType = UITableViewCellAccessoryNone;
            }else
            {
